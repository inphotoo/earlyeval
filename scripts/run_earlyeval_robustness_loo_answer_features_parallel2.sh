...
DATASETS="${DATASETS:-terminalbench}"
ONLY_TEST_MODELS="${ONLY_TEST_MODELS:-}"

MAX_PARALLEL="${EARLYEVAL_ROBUSTNESS_LOO_MAX_PARALLEL:-2}"
PARQUET_BATCH_SIZE="${SWE_ROBUSTNESS_LOO_BATCH_SIZE:-256}"
VMEM_KB="${SWE_ROBUSTNESS_LOO_VMEM_KB:-56623104}"
VMEM_BYTES="$((VMEM_KB * 1024))"
RSS_KILL_GB="${SWE_RSS_KILL_GB:-42}"
...
LOG_DIR="${OUTPUT_DIR}/logs/robustness_loo_answer_features_parallel2"
mkdir -p "${LOG_DIR}"
...
echo "[two-lanes] generating command_index for ${DATASETS}"
...
run_one_fold() {
  ...
  local selected_path="${fold_dir}/safe_stop_test_selected.csv"
  local runner_pid=""

  cleanup_child() {
    if [[ -n "${runner_pid}" ]] && kill -0 "${runner_pid}" 2>/dev/null; then
      echo "[two-lanes] cleanup kill dataset=${dataset} model=${model}" | tee -a "${fold_log}"
  ...
  if [[ -f "${success_path}" && -f "${selected_path}" ]]; then
    echo "[two-lanes] skip existing dataset=${dataset} model=${model}"
    return 0
  fi

  {
    echo ""
    echo "[two-lanes] start $(date '+%F %T') dataset=${dataset} model=${model}"
    echo "[two-lanes] settings parallel=${MAX_PARALLEL} batch=${PARQUET_BATCH_SIZE} vmem_kb=${VMEM_KB} rss_kill_gb=${RSS_KILL_GB} timeout=${FOLD_TIMEOUT}"
  } | tee -a "${fold_log}"
  ...
  while kill -0 "${runner_pid}" 2>/dev/null; do
    ...
    if (( rss_kb > RSS_KILL_KB )); then
      ...
      {
        echo "[two-lanes] RSS watchdog kill at $(date '+%F %T'): rss_kb=${rss_kb} limit_kb=${RSS_KILL_KB}"
      } | tee -a "${fold_log}"
      kill -TERM "-${runner_pid}" 2>/dev/null || true
      sleep 120
      kill -KILL "-${runner_pid}" 2>/dev/null || true
  ...
  {
    echo "[two-lanes] end $(date '+%F %T') dataset=${dataset} model=${model} status=${status} max_rss_kb=${max_rss_kb} killed_by_watchdog=${killed_by_watchdog}"
  } | tee -a "${fold_log}"

  if [[ "${status}" != "0" ]]; then
    echo "[two-lanes] FAILED dataset=${dataset} model=${model}; log=${fold_log}" >&2
    return "${status}"
  fi
}
...
while IFS=$'\t' read -r dataset model fold_dir safe_model; do
  ...
  if (( failures > 0 )) && [[ "${STOP_ON_FAILURE}" == "1" ]]; then
    break
  fi
...
if (( failures > 0 )); then
  echo "[two-lanes] failed jobs=${failures}" >&2
  exit 1
fi

echo "[two-lanes] refreshing command_index"
"${PYTHON_BIN}" -m earlyeval.experiments.robustness_15pct \
  --config configs/earlyeval.yaml \
  --output-dir "${OUTPUT_DIR}" \
  --run-subdir "${RUN_SUBDIR}" \
...
echo "[two-lanes] done $(date '+%F %T') success_count=${success_count}"
