...
CACHE_ROOT="${CACHE_ROOT:-${PACKAGE_ROOT}/paper/experiments/earlyeval_lightgbm/model_compare/llm_logit_${MODEL_TAG}_sweverify_cache}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${PACKAGE_ROOT}/paper/experiments/earlyeval_lightgbm/model_compare/llm_logit_${MODEL_TAG}_sweverify}"
...
for model_id in ${TEST_MODELS}; do
  ...
  (
    ...
    "${PYTHON_BIN}" "${PACKAGE_ROOT}/earlyeval/vendor/architecture_baselines/llm_logit_baselines/run_local_llm_logits.py" \
      --cache-dir "${cache_dir}" \
      --output-dir "${fold_dir}" \
