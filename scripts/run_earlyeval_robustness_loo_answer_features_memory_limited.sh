...
LOG_DIR="${OUTPUT_DIR}/logs/robustness_loo_answer_features_memory_limited"
mkdir -p "${LOG_DIR}"
...
The answer-feature robustness preset is heavy on parquet reads/writes. Running near a full filesystem can
...
This script refuses to run the answer-feature preset without a kernel-level MemoryMax cap.
...
run_one_fold() {
  ...
  echo "[robustness-loo] $(date '+%F %T') dataset=${dataset} model=${model}" | tee -a "${fold_log}"
...
while IFS=$'\t' read -r dataset model; do
  ...
  if [[ -f "${success_path}" ]]; then
    echo "[robustness-loo] skip existing dataset=${dataset} model=${model}"
    continue
  fi
...
echo "[robustness-loo] done $(date '+%F %T')"
