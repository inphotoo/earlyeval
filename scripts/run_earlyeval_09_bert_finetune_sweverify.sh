...
CACHE_ROOT="${CACHE_ROOT:-${PACKAGE_ROOT}/paper/experiments/earlyeval_lightgbm/model_compare/bert_codebert_finetune_sweverify_cache}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${PACKAGE_ROOT}/paper/experiments/earlyeval_lightgbm/model_compare/bert_codebert_finetune_sweverify_e3}"
...
for model_id in ${TEST_MODELS}; do
  ...
  (
    cd "${REPO_ROOT}"
    "${PYTHON_BIN}" "${PACKAGE_ROOT}/earlyeval/vendor/architecture_baselines/bert_baselines/build_bert_embedding_cache.py" \
    ...
    "${PYTHON_BIN}" "${PACKAGE_ROOT}/earlyeval/vendor/architecture_baselines/bert_baselines/train_bert_finetune_dual_head.py" \
      --cache-dir "${cache_dir}" \
      --output-dir "${fold_dir}" \
      --encoder-name "${ENCODER_NAME}" \
      --device "${DEVICE}" \
