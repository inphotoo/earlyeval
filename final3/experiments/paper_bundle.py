...
from final3.checks.preflight import CURRENT_PAPER_FIGURES, CURRENT_PAPER_TABLES
from final3.core.io import ensure_dir, write_json, write_table
from final3.core.paths import load_paths, package_root, resolve_repo_path
...
def _current_dataset_sources() -> list[tuple[str, Path]]:
    cfg_path = package_root() / "configs" / "rq_final.yaml"
    ...
    *,
    ...
    in_package_sources: list[tuple[str, str, Path]] = [
        ...
        (
            ...
            "rq_final_lightgbm_17",
            package_root() / "paper" / "experiments" / "rq_final_lightgbm_17",
        ),
        ("config", "rq_final.yaml", package_root() / "configs" / "rq_final.yaml"),
    ]
...
def _write_readme(path: Path, summary: dict[str, Any]) -> None:
    lines = [
        "# Paper Data Inputs",
        "",
        "This directory is managed by `python -m final3.cli experiment materialize-paper`.",
