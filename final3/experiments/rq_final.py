...
class RqFinalConfig:
    ...
    def run_id(self) -> str:
        ...
        return str(smoke.get("run_id") or run.get("default_run_id") or "rq_final_smoke")
...
def load_rq_final_config(config: str | Path = "configs/rq_final.yaml") -> RqFinalConfig:
    cfg_path = _resolve_project_path(config)
    payload = yaml.safe_load(cfg_path.read_text(encoding="utf-8")) or {}
    return RqFinalConfig(path=cfg_path, payload=payload)


def _default_output_dir(cfg: RqFinalConfig, run_id: str | None = None) -> Path:
...
def _dataset_items(cfg: RqFinalConfig, names: list[str] | None = None) -> list[tuple[str, dict[str, Any]]]:
    ...
    if missing:
        raise KeyError(f"Unknown rq_final dataset(s): {', '.join(missing)}")
...
def build_execution_plan(config: str | Path = "configs/rq_final.yaml", output_dir: str | Path | None = None) -> dict[str, Any]:
    ...
    plan = {
        ...
        "stages": [
            {"id": "00_preflight", "command": "python -m final3.cli check preflight --experiment all"},
            {"id": "01_prefix_audit", "command": "python -m final3.cli experiment rq-final --stage audit-prefix"},
            {"id": "02_make_splits", "command": "python -m final3.cli experiment rq-final --stage make-splits"},
            {"id": "03_main_lightgbm", "status": "planned_heavy", "dataset_scope": "sweverify, toolathlon, terminalbench"},
            {"id": "04_lightgbm_valid_accuracy_sweep", "command": "python -m final3.cli experiment rq-final --stage lightgbm-policy-sweep"},
            ...
            {"id": "08_paper_refresh", "status": "planned", "output": "paper/results/rq_final_<run_id>"},
        ],
        "datasets": {
            name: {
                "role": spec.get("role"),
...
def audit_prefix_tables(
    config: str | Path = "configs/rq_final.yaml",
...
def make_split_manifests(
    ...
    - excluded_train_models from `configs/rq_final.yaml` (matches what
...
def _trainer_path(cfg: RqFinalConfig) -> Path:
    return _answer_module_root() / "safe_stop_dual_head_retrain.py"


def _legacy_trainer_path(cfg: RqFinalConfig) -> Path:
    """Backward-compatible alias for older final3 experiment helpers."""

    return _trainer_path(cfg)


def _excluded_models_from_config(cfg: RqFinalConfig) -> set[str]:
    split_cfg = cfg.payload.get("split") or {}
...
def _min_trajectories_per_model(cfg: RqFinalConfig) -> int:
    split_cfg = cfg.payload.get("split") or {}
...
def _safe_label_min_step(cfg: RqFinalConfig) -> int:
    main = cfg.payload.get("main_model") or {}
    return int(main.get("safe_label_min_step", _DEFAULT_SAFE_LABEL_MIN_STEP))


def _fit_feature_engineer_on_train(cfg: RqFinalConfig) -> bool:
...
def _eligible_lightgbm_folds(cfg: RqFinalConfig, dataset: str = "sweverify") -> list[dict[str, Any]]:
    import pyarrow.parquet as pq
    ...
    command = [
        ...
        *[str(v) for v in main.get("score_modes", [main.get("score_mode", "calibrated")])],
        ...
        str(
            int(
                os.environ.get(
                    "RQ_FINAL_LGBM_THREADS_PER_FOLD",
                    str(
                        resources.get(
...
def run_smoke(config: str | Path = "configs/rq_final.yaml", output_dir: str | Path | None = None) -> dict[str, Any]:
    cfg = load_rq_final_config(config)
    out = ensure_dir(output_dir or _default_output_dir(cfg, cfg.run_id))
    ...
    success.write_text("rq_final smoke completed\n", encoding="utf-8")
    return {
...
def run_rq_final_stage(
    *,
    ...
    raise ValueError(f"Unsupported rq_final stage: {stage}")
