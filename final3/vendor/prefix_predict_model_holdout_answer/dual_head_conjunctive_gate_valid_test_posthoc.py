...
def main() -> None:
    ...
    lines = [
        ...
        "",
        "当前 baseline 的 dual-head 逻辑是：`success_score >= ThrS` 或 `failure_score >= ThrF` 就可决策；两边都触发时按 margin 选边。",
        "这里新增的 conjunctive gate 更保守：`success >= thr` 且 `failure <= 1-thr` 才判 success；反之亦然。",
        "",
        "选阈值只看 validation；test 只做锁定评估。",
        "",
        f"整体上，conjunctive 相比 baseline 在 `{improvement_counts['better_drop']}/{len(merged)}` 个 run-score 组合上降低了 test drop，在 `{improvement_counts['better_acc']}` 个组合上提高了 test acc，在 `{improvement_counts['better_margin_auc']}` 个组合上提高了 margin AUC。",
        "",
    ...
    lines += [
        ...
        "",
        "筛选口径：`valid abs_drop_pp <= 2pp`、`valid decision_accuracy >= 90%`、`valid pct_steps_saved > 0`；否则回退到最小 valid abs_drop 再看 save。",
        "",
