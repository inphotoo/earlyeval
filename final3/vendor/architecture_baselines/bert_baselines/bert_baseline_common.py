...
def _package_root() -> Path:
    for parent in Path(__file__).resolve().parents:
        if (parent / "configs" / "rq_final.yaml").exists():
            return parent
    raise RuntimeError("Could not locate SweBench_Organized_Package_final3 root")


PACKAGE_ROOT = _package_root()
REPO_ROOT = PACKAGE_ROOT.parent
PROJECT_ROOT = PACKAGE_ROOT / "final3" / "vendor" / "prefix_predict_model_holdout_answer"
...
DEFAULT_RESULTS_ROOT = PACKAGE_ROOT / "paper" / "experiments" / "rq_final_architecture_smoke"
