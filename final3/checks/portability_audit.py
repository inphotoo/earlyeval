...
def _scan_external_code_refs() -> list[dict[str, Any]]:
    ...
    for path in package_root().rglob("*"):
        ...
        for pattern in EXTERNAL_CODE_PATTERNS:
            ...
            rel_path = _rel(path)
            if rel_path in {"final3/checks/portability_audit.py"}:
                continue
            status = "historical_optional" if rel_path.startswith("scripts/run_rq_final_09_") else "review"
            if rel_path.startswith("configs/") and "experiment_registry" in rel_path:
                status = "historical_result_reference"
            if rel_path in {"configs/paths.yaml", "configs/paths.example.yaml", "final3/core/paths.py"}:
...
def _write_readme(
    ...
    *,
    ...
    lines = [
        "# final3 Portability Audit",
        "",
        "Scope: mainline code should execute from `final3/`; historical results are frozen; large data/model files stay external and are checked by manifest.",
        "",
        "## Summary",
        "",
        ...
        "- `external_artifacts.csv`: heavyweight data/model files kept outside final3, with size and optional sha256.",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Audit final3 portability boundaries.")
