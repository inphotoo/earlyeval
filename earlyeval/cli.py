...
from earlyeval.benchmarks.normalize import normalize_file
from earlyeval.checks.preflight import run_preflight
from earlyeval.experiments.paper_bundle import materialize_paper_inputs
from earlyeval.experiments.registry import list_experiments
from earlyeval.experiments.paper_pipeline import run_earlyeval_stage
from earlyeval.legacy.wrappers import explain_legacy_entry
from earlyeval.models.dual_head_lightgbm import DualHeadRunSpec, run_dual_head
from earlyeval.models.heavy import describe_heavy_experiments
from earlyeval.pipelines.current_safe_stop import run_current_safe_stop
from earlyeval.policies.apply import apply_policy_to_file
from earlyeval.reports.paper_tables import refresh_paper_tables
...
def build_parser() -> argparse.ArgumentParser:
    """Build the top-level earlyeval command-line parser."""

    parser = argparse.ArgumentParser(
        prog="python -m earlyeval.cli",
        description="EarlyEval maintainable source entrypoints for SWE-bench safe-stop experiments.",
    )
    ...
    normalize = data_sub.add_parser("normalize", help="Normalize raw benchmark records into earlyeval trajectory contract.")
    ...
    experiment = sub.add_parser("experiment", help="List or prepare experiment sets owned by earlyeval.")
    ...
    materialize = experiment_sub.add_parser("materialize-paper", help="Put paper inputs under earlyeval/paper/data.")
    ...
    earlyeval = experiment_sub.add_parser("paper-suite", help="Plan or smoke-test the paper experiment suite.")
    earlyeval.add_argument(
        "--stage",
        choices=(
            "plan",
            "audit-prefix",
    ...
    earlyeval.add_argument("--config", type=Path, default=Path("configs/earlyeval.yaml"))
    earlyeval.add_argument("--output-dir", type=Path, default=None)
    earlyeval.add_argument("--datasets", nargs="*", default=None)
    earlyeval.add_argument("--max-folds", type=int, default=None)
    earlyeval.add_argument("--execute", action="store_true", help="Run heavy stages instead of writing a dry-run plan.")
    earlyeval.add_argument("--force", action="store_true", help="Re-run folds even when output markers already exist.")
    earlyeval.add_argument(
        "--max-parallel-folds",
        type=int,
        default=1,
    ...
    dual.add_argument("--output-subdir", default="earlyeval_current_safe_stop_dual_head")
...
def _dispatch(args: argparse.Namespace) -> int:
    ...
    if args.command == "experiment" and args.experiment_command == "materialize-paper":
        ...
        return 0
    if args.command == "experiment" and args.experiment_command == "paper-suite":
        _print_result(
            run_earlyeval_stage(
                stage=args.stage,
                config=args.config,
...
def main(argv: list[str] | None = None) -> int:
    """Run the earlyeval CLI and convert expected user errors to one-line messages."""
